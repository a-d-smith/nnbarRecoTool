...
#
SUBDIRS := findVertex pi0RecoTool nnbarRecoTool #ADD_NEW_SUBDIR ... do not remove this comment from this line
...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/nnbarRecoTool/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/nnbarRecoTool/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/nnbarRecoTool/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/nnbarRecoTool/doc/doxygenMyProject.script
